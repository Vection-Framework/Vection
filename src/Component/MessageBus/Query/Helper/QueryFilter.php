...
class QueryFilter implements QueryFilterInterface
{
    ...
    public function apply(string $filter): void
    {
        ...
        $validate = '/\(?([\w-]+.{0,1})\s?(=|<=|>=|<|>|(IS\s+(NOT\s+)?(NULL)?)|(NOT\s+)?LIKE)\s?"(\w*)"\)?\s?(AND|OR)?/iU';
        ...
        $validate = '/([\w-]+.{0,1})\s?(=|<=|>=|<|>|(IS\s+(NOT\s+)?(NULL)?)|(NOT\s+)?LIKE)\s?/iU';
        /** @var string $filter */
        $filter = \preg_replace_callback($validate, function($match){
